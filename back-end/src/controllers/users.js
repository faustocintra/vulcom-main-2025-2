...
controller.me = async function(req, res) {
  try {
    ...
    // e os dados do usuário estão disponíveis em req.authUser
    ...
    const user = await prisma.user.findUnique({
      select: {
        ...
        username: true,
        email: true,
        // outros campos que você queira retornar (exceto password)
      },
    ...
    if (user) {
      ...
      // Usuário não encontrado no BD (pode ter sido deletado)
      // HTTP 404: Not Found
      res.status(404).end()
    }
  }
